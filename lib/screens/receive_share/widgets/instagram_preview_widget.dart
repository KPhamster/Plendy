...
class InstagramWebViewState extends State<InstagramWebView> {
  ...
  Future<void> _initializeOEmbed() async {
    ...
    try {
      ...
      final normalizedUrl = _normalizeInstagramUrl(widget.url, convertReelToPost: false);
      ...
      if (!oembedService.isConfigured) {
        ...
        setState(() {
          _errorMessage = 'Instagram API not configured. Please configure Facebook App credentials.';
          isLoading = false;
        });
        return;
      }
...
''';
  ...
  Widget build(BuildContext context) {
    ...
    final webViewKey = ValueKey('ig_${widget.url}_${loadOEmbedHtml ? 'oembed' : 'direct'}_${_oembedHtml?.hashCode ?? 0}');
        ...
        Container(
          ...
          child: InAppWebView(
            ...
            initialUrlRequest: loadOEmbedHtml 
                ? null 
                : URLRequest(url: WebUri(_normalizeInstagramUrl(widget.url, convertReelToPost: true))),
